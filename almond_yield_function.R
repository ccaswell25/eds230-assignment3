...
#' @param df a dataframe consisting of the following variables : year, month, minimum daily temperature (celsius), maximum daily temperature (celsius), daily precipitation (mm)

#' @return minimum yield (ton/ acre), maximum yield (ton/acre), mean yield (ton/acre)
#' 
...
# almond yield anamoly formula utilizing febuary tempearture values and january precipitation values
  almond_yield_calculation <- full_join(feb_min_temp, jan_mean_precip) %>%  mutate( almond_yield_calculation = Tmincoeff1 * feb_min_temp$min_temp + Tmincoeff2*feb_min_temp$min_temp^2 + Pcoeff1 * jan_mean_precip$total_precip + Pcoeff2 * jan_mean_precip$total_precip^2 + intercept)
